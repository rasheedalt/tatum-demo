...
export default class TRON implements BlockchainBase{
    private sdk: any;

    constructor(){
        this.sdk = TatumTronSDK({ apiKey: process.env.API_KEY})
    }
    ...
    getVirtualAccountInfo = async (accountId: string) => {
        const accounts = await this.sdk.ledger.account.get(accountId)
        console.log(`Account info: ${JSON.stringify(accounts)}`)
        return accounts;
    };
} 